...
## FUCTIONS ##
usage() {
    echo "mangoJI Server startup/down script, rel. 2015.10.27."
    echo "Usage:"
...
PID=`jps | grep MangoJI | awk '{print $1}'`
...
case "$1" in
  start)
       if [ ! -z $PID ]; then
           echo "mangoJI already started.($PID)"
           exit 1
       fi 
      nohup $JAVA -classpath $CLASSPATH $JAVAOPTION $JAVA_OPTS app.kit.MangoJI $2 $3 &
#      $JAVA -classpath $CLASSPATH $JAVAOPTION $JAVA_OPTS app.kit.MangoJI $2 $3
      echo "mangoJI Server are started."
  ;;

  stop)
      kill -15 $PID
      echo "mangoJI process are finish.($PID)"
      ;;
