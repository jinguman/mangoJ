...
## FUCTIONS ##
usage() {
    echo "mangoJF Server startup/down script, rel. 2015.10.27."
    echo "Usage:"
...
PID=`jps | grep MangoJF | awk '{print $1}'`
...
case "$1" in
  start)
       if [ ! -z $PID ]; then
           echo "mangoJF already started.($PID)"
           exit 1
       fi 
      nohup $JAVA -classpath $CLASSPATH $JAVAOPTION $JAVA_OPTS app.kit.MangoJF &
#      $JAVA -classpath $CLASSPATH $JAVAOPTION $JAVA_OPTS app.kit.MangoJF
      echo "mangoJF Server are started."
  ;;

  stop)
      kill -15 $PID
      echo "mangoJF process are finish.($PID)"
      ;;
